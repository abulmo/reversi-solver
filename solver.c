...
/*!
 * \brief Count flipped on the board.
 *
 ...
 */
int board_count_flips(Board *board, int x, const char p)
{
	const char o = OPPONENT(p);
...
int board_get_final_score(const Board *board)
{
	...
	if (score > 0) score += board->n_empties; else if (score < 0) score -= board->n_empties;
 ...
 */
void PVS_root(Board *board, HashTable *hash_table, int alpha, int beta, int depth,
	MoveList *movelist)
{
	int lower, upper;
	MoveList *iter;
 ...
 */
void solve(Board *board, HashTable *hash_table, int depth, Move *bestmove)
{
	int score, bound;
	...
	if (movelist->next != NULL) { /* normal play ? */
		if (board->n_empties > EMPTIES_DEEP_TO_SHALLOW_SEARCH)
			movelist_sort_fastfirst(movelist, board);
		PVS_root(board, hash_table, -1, +1, depth, movelist);
		*bestmove = movelist->next->move;
		...
		if (movelist->next == NULL) { /* game over ? */
			...
			*bestmove->position = NOMOVE;
			...
			if (board->n_empties > EMPTIES_DEEP_TO_SHALLOW_SEARCH)
				movelist_sort_fastfirst(movelist, board);
			PVS_root(board, hash_table, -1, +1, depth - 1, movelist);
...
#endif
		solve(&board, &hash_table, board.n_empties, &bestmove);
		t = clock() - t;
		T += t;
		nodes += board.n_nodes;
		if (verbose) {
